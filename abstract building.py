...
file = 'building/building-violations.csv'
log_path = 'log.txt'
...
def init():
    ...
    building_cate = data.columns #每个属性
   
    #获取标称属性
    nominalCol=['VIOLATION LAST MODIFIED DATE', 'VIOLATION DATE',
    ...
    #获取数值属性
    chemicalCol=['INSPECTION NUMBER', 'PROPERTY GROUP', 'Community Areas', 'Census Tracts','Wards','Historical Wards 2003-2015']
...
def set_missing_Forest(data, cols):

    data_lost3 = data.copy()

    for col in cols:
        # 把已有的数值型特征取出来丢进Random Forest Regressor中
        price_df = data_lost3[['INSPECTION NUMBER', 'PROPERTY GROUP', col]]

        # 乘客分成已知年龄和未知年龄两部分
        known_price = price_df[price_df[col].notnull()].values
        unknown_price = price_df[price_df[col].isnull()].values
        ...
        #     print predictedAges
        # 用得到的预测结果填补原缺失数据
        data_lost3.loc[ (data_lost3[col].isnull()), col ] = predictedprice
...
if __name__=="__main__":
    #打开excel获取数据
    (data,nominalCol,chemicalCol)=init(file)
    ...
    # 3.相关关系来填补缺失值
    # new_drop_3 = set_missing_Forest(data)
    #可视化
    # col = 'price'
